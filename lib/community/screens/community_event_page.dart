...
class _CommunityEventsPageState extends State<CommunityEventsPage> {
  ...
  // --- WIDGET BARU: DRAWER (Persis Gambar) ---
  Widget _buildDrawer(BuildContext context) {
    const Color headerColor = Color(0xFF005960); // Hijau Tua sesuai gambar
    ...
    return Drawer(
      child: Container(
        ...
        child: Column(
          children: [
            // Header Drawer "Menu" + Close Button
            Container(
            ...
            _buildDrawerItem(Icons.home, "Home", onTap: () {
              Navigator.pushNamed(context, '/home'); // Sesuaikan routemu
            }),
            _buildDrawerItem(Icons.groups, "Community", onTap: () {
              // Karena kita sudah di sub-bagian community, bisa pop atau push replacement
              Navigator.pushNamed(context, '/community');
            }),
            _buildDrawerItem(Icons.store, "Store", onTap: null, trailing: "Soon"),
            _buildDrawerItem(Icons.calendar_today, "Booking", onTap: () {
  ...
  Widget _buildDrawerItem(IconData icon, String title, {VoidCallback? onTap, String? trailing}) {
    const Color iconColor = Color(0xFF005960); // Warna Icon Hijau Tua
    ...
    return ListTile(
      ...
      title: Text(
        title,
        style: GoogleFonts.inter(
          color: const Color(0xFF002147), // Biru Tua Gelap
  ...
  // --- WIDGET BARU: USER POPUP (Persis Gambar) ---
  void _showUserMenu(BuildContext context) {
    final request = context.read<CookieRequest>();
    // BUG FIX: Ambil username dari jsonData, bukan string statis "User"
    String username = request.jsonData['username'] ?? 'User';
    const Color primaryColor = Color(0xFF005960);

    showMenu(
      context: context,
      position: RelativeRect.fromLTRB(MediaQuery.of(context).size.width, 80, 0, 0), // Kanan atas
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
      color: const Color(0xFFE8EFF5), // Warna background abu muda/biru muda sesuai gambar
        ...
        PopupMenuItem(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                username, // Nama User Dinamis
                style: GoogleFonts.inter(fontWeight: FontWeight.bold, color: const Color(0xFF002147), fontSize: 14),
              ),
            ],
          ),
        ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // DRAWER SUDAH DIGANTI
      drawer: _buildDrawer(context),

      appBar: AppBar(
        backgroundColor: primaryNavColor,
        elevation: 0,
        // Tombol Hamburger otomatis memanggil drawer
        title: Text('GETFIT.TODAY', style: GoogleFonts.inter(fontWeight: FontWeight.w900, color: Colors.white)),
        actions: [
          // Tombol User/Avatar
          GestureDetector(
            onTap: () => _showUserMenu(context), // Panggil pop-up user
            child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        TextField(
                          ...
                          decoration: _inputDecoration("Cari nama event..."),
                        ),
                          ...
                          children: [
                            ...
                            Expanded(
                              child: ElevatedButton(
                                ...
                                style: ElevatedButton.styleFrom(backgroundColor: accentYellow, foregroundColor: Colors.black, padding: const EdgeInsets.symmetric(vertical: 12)),
